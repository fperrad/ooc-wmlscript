...
langLib put(13, func(interp: Interp) -> WmlsAny {
    ...
    if (val instanceOf?(WmlsInvalid))
        return WmlsInvalid new()
    seed := val instanceOf?(WmlsFloat) ? val _toFloat() as Int : val _toInt()
...
floatLib put(0, func(interp: Interp) -> WmlsAny {
    ...
    if (val instanceOf?(WmlsString))
        val = val as WmlsString parseNumber()
    if (val instanceOf?(WmlsFloat))
        return WmlsInteger new(val _toFloat() as Int)
    else if (val instanceOf?(WmlsInteger) || val instanceOf?(WmlsBoolean))
        return WmlsInteger new(val _toInt())
    else
        return WmlsInvalid new()
})
...
stringLib put(2, func(interp: Interp) -> WmlsAny {
    ...
    s := str _toString()
    len := s length()
    i := idx instanceOf?(WmlsFloat) ? idx _toFloat() as Int : idx _toInt()
